...
ADAPTER_RATE_PATTERNS='rate limit|rate_limit|rate-limit|(^|[^0-9])429([^0-9]|$)|too many requests|quota|out of credits|insufficient credits'
ADAPTER_AUTH_PATTERNS='(^|[^0-9])401([^0-9]|$)|unauthorized|unauthenticated|not logged in|login required|invalid api key|AMP_API_KEY'
